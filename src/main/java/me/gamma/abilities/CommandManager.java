...
public class CommandManager implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!cmd.getName().equalsIgnoreCase("abilities")) return true;
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "reload":
                if (!checkPermission(sender, "abilities.admin")) return true;
                plugin.reloadConfig();
    ...
    // Método para enviar mensajes de lista o únicos
    private void sendConfigMessage(CommandSender sender, String path) {
        if (plugin.getConfig().isList(path)) {
            ...
            sender.sendMessage(plugin.color(plugin.getConfig().getString(path, "&cError: Mensaje no encontrado en config: " + path)));
        }
    }

    // Método auxiliar para reemplazar variables comunes
    private void sendFormattedMessage(CommandSender sender, String path, String item, int amount, String targetName) {
        String msg = plugin.getConfig().getString(path, "");
        if (item != null) msg = msg.replace("<item>", item);
        if (amount > 0) msg = msg.replace("<amount>", String.valueOf(amount));
        if (targetName != null) msg = msg.replace("<target>", targetName).replace("<player>", targetName);
    ...
    private ItemStack getItemByName(String name) {
        switch (name.toLowerCase()) {
            case "vampire": return VampireItem.create(plugin);
            case "mixer": return MixerItem.create(plugin);
            case "antifall": return AntiFallItem.create(plugin);
            case "helmet": return HelmetRemoverItem.create(plugin);
            case "rocket": return RocketItem.create(plugin);
            case "snowball": return SnowballItem.create(plugin);
            case "switcher": return SwitcherItem.create(plugin);
            default: return null;
        }
    }
}